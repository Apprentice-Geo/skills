...
- Skill.md 正文使用结构化组织方式，至少包含对应 Skill 的适用范围、核心规则等 Skill 正常使用的必需信息。
- 指令简洁明确，并优先使用表格和“场景-正确行为-禁止行为”的描述结构。
- 优先写具体情景下的具体行为规则，不写冗长的背景或通用知识说明。
- 保证内容的单源性，同一内容使用引用而不是在多个文档重复维护。
- 代码架构及行为、常见错误处理方式等非必需信息放在 references 文件夹的文档中，在 Skill.md 正文只简单说明并引用对应文档。
- 模板、图片、字体、指令等资源统一放在 assets 文件夹下。

## 编辑规则

- 以仓库中的 Skill 为第一可信状态源，而不是记忆中的状态或者已安装 Skill 的状态。
- 完成 Skill 的输入输出、配置格式、目录结构、命令或执行流程等用户可见行为修改后，必须检查并同步更新相关文档。
- 删除或废弃功能时，同时删除对应说明、示例和引用。
- 保留无关的本地修改和未跟踪文件，保留代码中的正确注释。
- 不得仅追加新规则来覆盖旧规则,存在冲突时，应修改或删除旧规则，使当前有效规则唯一且明确。
...
- `article-format-correction` 是 Markdown 校正 Skill。它保留原文的含义、结构、表达风格和明确编辑范围，并基于 Skill 作者的个人偏好修改 Markdown 格式。
- `anti-sycophancy-review` 是严格审查 Skill。它要求 Agent 不迎合用户、质疑缺乏支撑的假设并基于标签说明置信度。
- `audio-transcribe` 是本地音频转写 Skill。它接收本地音频路径并完成转写，不处理音频下载或转写后的其它处理流程，它可以和其它 Skill 组合使用达成对音频的综合处理。
- `bili-audiosummary` 是B站视频总结 Skill。它接收B站视频 URL 并完成视频下载等流程，然后等待音频转写结果并生成总结指令，最后由 Agent 根据指令完成总结。
