...
- 公共结果由 `manifest.json` 和 `transcript.json` 组成，可一起复制或移动，无需携带日志、workspace 或原音频。`manifest.json` 是唯一公开入口，必须通过 `audio_transcribe_contract.load_result()` 验证后读取。
- 当前公共格式、兼容边界和损坏恢复方式见[架构说明](references/ARCHITECTURE.md#公共-contract-与发布)。需要固定历史结果时保存独立 bundle；公共 loader 始终只读。
...
通用依赖源按官方 PyPI、清华、阿里的顺序配置，并使用 uv 的 `first-index` 策略；国内源作为可信的后续候选源。PyTorch 使用独立的官方 explicit index：默认 `cpu` extra 选择 CPU wheel，`qwen3-asr` extra 选择 CUDA 12.6 wheel。两组 extra 互斥，禁止同时启用或使用 `--all-extras`。
