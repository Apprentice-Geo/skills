...
The resolved request must contain the exact supported alignment policy. It participates in `variant_id`; manifests created before this requirement, or manifests with a missing or modified policy, are invalid and must be regenerated by retranscribing the audio. Raw timestamps are strictly ordered, non-overlapping, and satisfy `0 <= start < end <= duration`.
...
Do not claim a transcript was produced unless a complete manifest is present and validates successfully. Do not repair, clip, or coerce invalid public timestamps or workspace data. Once the command resolves a Provider, later loading, inference, alignment, or publication failures stop the run; do not silently switch Providers.
