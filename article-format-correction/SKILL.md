...
license: Apache-2.0
metadata:
  Github: https://github.com/Apprentice-Geo/skills/tree/main/article-format-correction
---
...
- 违反本节后续格式规范的问题。
- 当预期的 Markdown 格式明确时，修正格式错误的标题、列表、表格、链接、图片或围栏代码块。
...
- 全角标点和其它字符之间不加空格。
- 中文句子中使用全角中文标点，但是遇到完整的英文整句、特殊名词时，其内容使用半角标点。
- 专有名词使用正确的大小写，例如 GitHub、Google、ChatGPT 等。
- 不使用不地道的缩写，例如 TypeScript 不应写作 Ts，HTML5 不应写作 h5。

例外情况：

- 度数、百分比与数字之间不加空格，例如 90°，15%。
- 中文句子内有英文书籍名、英文报刊名时，不使用中文书名号，而应使用英文斜体表示，在 Markdown 格式中可使用斜体标记，例如 `*Nature*`。

除非用户另有指定，否则应用以下 Skill 作者的偏好：

- 中文句子中使用 `「」`,`『』`,`[]` 和半角空格，不使用 `“”`,`''`,`【】` 和全角空格。
...
如果术语、命令、路径、版本、公式或技术陈述可能有误，但无法确信如何修正，则保持原样，不得猜测，并在最终输出中的 `需确认项` 下列出该内容及简要原因。
