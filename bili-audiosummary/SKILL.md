...
The pipeline prefers usable subtitles and otherwise transcribes audio. faster-whisper is the default ASR provider. On a CUDA-capable machine with the optional dependencies and models installed, Qwen3-ASR can be selected explicitly. Provider selection is strict: a Qwen3 dependency, model, CUDA, loading, inference, or alignment failure ends the transcription and never falls back to faster-whisper. Before use, install the model required by the selected provider.
...
Both ASR providers decode the source once to shared 16kHz mono float32 samples and plan continuous chunks in integer sample coordinates. They do not create chunk WAV files. A complete cache hit returns before decode or model loading; a partial resume decodes once and reuses valid per-chunk results.

The global planning VAD uses a 300ms minimum silence and full silence windows as legal boundaries. Normal chunks are 30-180 seconds; audio shorter than 30 seconds uses one chunk. Whisper Schema 6 keeps worker-divisible chunk counts and passes only `vad_filter=True`. Qwen3 Schema 2 uses the `QWEN3_MAX_INFERENCE_BATCH_SIZE`-driven `full` count rule, batches ndarray slices, retains strict no-fallback semantics, and uses `max_new_tokens=1024`.
