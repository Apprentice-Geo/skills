...
- `scripts/setup/install_qwen3.py`: install optional CUDA Qwen3 dependencies and download both required Qwen3 models.
  Setup keeps complete command output in `.cache/logs/setup-YYYYMMDD-HHMMSS.log`, while the terminal only shows short progress messages and failed-command output. Existing `UV_CACHE_DIR`, `HF_HOME`, `HUGGINGFACE_HUB_CACHE`, `PIP_INDEX_URL`, and `HF_ENDPOINT` values are preserved.
