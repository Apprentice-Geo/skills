...
- Both `asr_parallel/` and `asr_qwen3/` use `asr_plan.json`, `vad_result.json`, `progress.json`, `chunk_results/`, `result.json`, and `metrics.json`. `vad_result.json` is a required recoverable cache artifact: its schema, source identity, VAD parameters, and strictly ordered non-overlapping integer sample intervals are validated without normalization. The current policy must reproduce the plan's complete chunk layouts from those intervals.
- A complete fast-path cache requires a matching plan, a valid VAD artifact whose derived layouts exactly match that plan, and every valid chunk result; only then does it skip audio decode, device checks, and model loading. A valid partial cache decodes once without repeating VAD, rebuilds progress, and processes only missing/invalid chunks.
- Missing, unreadable, incompatible, or structurally invalid VAD artifacts trigger one decode and a new VAD pass. If the regenerated layouts exactly match the cached plan, the pipeline rewrites `vad_result.json` and retains valid chunk results; otherwise it writes a new plan and rejects all old chunk results. A valid VAD artifact whose current-policy layouts differ from the cached plan rebuilds the plan directly from those intervals and likewise rejects old chunk results. If decoded sample count differs from cached source identity, VAD and planning are regenerated.
