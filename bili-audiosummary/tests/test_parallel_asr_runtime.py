...
def fake_audio(seconds: int) -> NormalizedAudio:
    return NormalizedAudio(np.broadcast_to(np.zeros(1, dtype=np.float32), seconds * SAMPLE_RATE))


def install_whisper(monkeypatch, model_class) -> None:
    monkeypatch.setitem(sys.modules, "faster_whisper", types.SimpleNamespace(WhisperModel=model_class))
...
def test_whisper_full_cache_skips_decode_vad_and_model(
    ...
    options = TranscribeOptions(model="model-dir", language="zh", num_workers=1, cpu_threads=1)
    monkeypatch.setattr(runner.os, "cpu_count", lambda: 4)
    monkeypatch.setattr(runner, "decode_normalized_audio", lambda _path: fake_audio(120))
    ...
    monkeypatch.setattr(runner, "decode_normalized_audio", lambda *_args: pytest.fail("decoded"))
    monkeypatch.setattr(runner, "detect_speech_intervals", lambda *_args: pytest.fail("VAD"))
    install_whisper(monkeypatch, type("Forbidden", (), {"__init__": lambda *_args, **_kwargs: pytest.fail("model") }))
    runner.run_parallel_whisper_transcribe(audio_path, options, workspace)
...
def test_whisper_partial_resume_decodes_once_and_uses_concurrent_ndarray_slices(
    ...
    audio_path.write_bytes(b"audio")
    workspace = workspace_tmp_path / "asr_parallel"
    options = TranscribeOptions(model="model-dir", language="zh", num_workers=2, cpu_threads=1)
    decoded = []
    inputs = []
    monkeypatch.setattr(runner.os, "cpu_count", lambda: 4)
    monkeypatch.setattr(runner, "decode_normalized_audio", lambda path: decoded.append(path) or fake_audio(360))
    monkeypatch.setattr(runner, "detect_speech_intervals", lambda *_args: [])
    ...
    assert len(inputs) == 2
    assert all(isinstance(samples, np.ndarray) and kwargs["vad_filter"] is True for samples, kwargs in inputs)
...
def test_whisper_chunk_result_uses_sample_identity(workspace_tmp_path: Path, monkeypatch) -> None:
    audio_path = workspace_tmp_path / "audio.m4a"
    audio_path.write_bytes(b"audio")
    workspace = workspace_tmp_path / "asr_parallel"
    options = TranscribeOptions(model="model-dir", language="zh", num_workers=1, cpu_threads=1)
