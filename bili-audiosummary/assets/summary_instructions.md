...
- 如果 transcript 足以支持整体请求，但局部内容依赖缺失的画面信息，只总结音频支持的部分，并在必需的限制说明 section 中指出无法确认的具体内容。
- 如果 transcript 整体不足以支持用户请求，停止写入总结并向调用方报告此 workflow 不执行画面分析；不得用笼统总结掩盖输入不足。
...
- 写入完成前，确认所有必需 section 已填写、重要内容有 transcript 支持，且使用的 timestamp 对应相关内容。completion 脚本不能替代这些内容检查。
